...
`hardware/frame/tello_style/preview/v2_*.png`, generated by the
`assembly_site_plain.scad` (iso/top/front/side/underside), `_exploded.scad`
(exploded/assembly) and `_section.scad` (battery section — translucent
shells, no boolean cut, since OpenSCAD drops per-object colour on newly-cut
boolean faces) scenes in that directory. Studio previews use the `Cornfield`
colorscheme (light background) for consistency with the dark page's white
`.ph` card boxes, and the body/capot colours are `color()`-wrapped explicitly
(grey `#9298a3` shell, blue `#2f6fed` capot — OpenSCAD's `--render` mode
drops per-object colours for these multi-part scenes; use plain preview
instead, no `--render` flag).
...
each motor with the same transform `sim/viz/gen_viewer.py` uses:
...
`assembly_site*.scad` scenes above, the capot/LED/front-prop/`--acc` colours
in `sim/viz/gen_viewer.py`, and `--acc`/`--acc2` in `site/style.css`.
