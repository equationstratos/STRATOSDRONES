...
OUT = os.path.join(PCB_DIR, "stratos_tinyhoop_aio.kicad_pcb")
...
# ---- explicit placement for major parts (mm; SQUARE whoop AIO board,
# origin at corner). x right (0..32), y down (0..32); y=0 is the FRONT edge
# (C6 antenna overhangs it). Whoop-style layout: P4 dead centre, C6 module at
# the front edge, one ESC cluster (EFM8+FD6288+6 FET) per corner next to its
# motor socket, power at the rear, USB on the left edge, CRSF/VTX pads on the
# right. Anchors UNTESTED in this sandbox (no pcbnew) — the CI kicad container
# run + assert_placement() arbitrate; grow BOARD to 35/36 if it fails.
PLACE = {
    "U3": (16.0, 6.2, 0, "T"),    # ESP32-C6 module — antenna over the FRONT edge
    "U1": (16.0, 17.0, 0, "T"),   # ESP32-P4 dead centre
    "U2": (7.0, 16.0, 90, "B"),   # flash (bottom, under P4 left)
    "Y1": (23.6, 13.4, 0, "T"),   # crystal, right of P4
    "U6": (23.6, 19.4, 0, "T"),   # IMU near board centre (vibration node)
    "U7": (21.0, 26.0, 0, "B"),   # baro (bottom, rear-right)
    "U5": (6.0, 26.0, 0, "T"),    # 3V3 buck, rear-left
    "L1": (9.6, 28.6, 0, "T"),
    "U11": (12.0, 26.0, 0, "T"),  # 5V buck, rear-centre-left
    "L3": (14.8, 28.6, 0, "T"),
    "U10": (26.0, 24.0, 0, "T"),  # P4 core DC-DC, rear-right
    "L2": (28.6, 26.6, 0, "T"),
    "J2": (2.8, 16.0, 90, "T"),   # USB-C, left edge
    "J1": (16.0, 30.4, 0, "T"),   # 2S battery pads, rear edge
    "J12": (29.4, 16.0, 90, "T"), # CRSF ELRS socket, right edge
    "J13": (29.4, 9.0, 90, "T"),  # VTX bay pads, right edge front
    "J14": (2.8, 8.0, 90, "T"),   # analog cam pads, left edge front
    "J4": (16.0, 24.0, 180, "B"), # OPTIONAL MIPI FFC (bottom, rear)
    "LED1": (4.0, 30.0, 0, "T"),
    "LED2": (28.0, 30.0, 0, "T"),
    "SW1": (30.0, 21.5, 90, "T"), # reset, right edge
    "SW2": (30.0, 26.5, 90, "T"), # boot, right edge
    "J10": (8.0, 30.6, 0, "B"),   # P4 uart pads (bottom rear)
    "J11": (24.0, 30.6, 0, "B"),  # C6 uart pads (bottom rear)
    # ESC clusters: EFM8 (top) + FD6288 (bottom) per corner, motor socket at
    # the corner edge. Mount holes live at (3.25,3.25)-pattern; clusters sit
    # inboard of them.
    "U20": (8.0, 8.6, 0, "T"),  "U30": (8.0, 8.0, 0, "B"),  "J5": (7.0, 1.6, 0, "T"),
    "U21": (24.0, 8.6, 0, "T"), "U31": (24.0, 8.0, 0, "B"), "J6": (25.0, 1.6, 0, "T"),
    "U22": (24.0, 22.0, 0, "T"),"U32": (24.0, 22.0, 0, "B"),"J7": (25.0, 30.4, 180, "T"),
    "U23": (8.0, 22.0, 0, "T"), "U33": (8.0, 22.0, 0, "B"), "J8": (7.0, 30.4, 180, "T"),
}
...
def inject_silk(path):
    ...
    # compact whoop-AIO silk: board name mid-front, FRONT arrow, motor corner
    # labels, CRSF/VTX pad labels on the right edge
    text("STRATOS TINYHOOP AIO", 8.2, 13.2, 0.9)
    text("FRONT ^", 12.4, 3.4, 0.8)
    for (x, y, lbl) in [(4.2, 5.4, "M1"), (26.2, 5.4, "M2"),
                        (26.2, 28.4, "M3"), (4.2, 28.4, "M4")]:
        text(lbl, x, y, 0.8)
    text("ELRS", 26.4, 14.0, 0.7, rot=90)
    text("VTX", 26.4, 7.6, 0.7, rot=90)
    text("2S", 18.6, 30.6, 0.8)
...
def inject_zones(path):
    ...
    for layer, nn in plan:
        if nn not in netnum:
            continue
        # No downward optical sensors on this board -> plain full pours.
