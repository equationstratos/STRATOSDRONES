#!/usr/bin/env python3
"""Fr4n8-001 (fpv85) browser 3-D viewer + PLAYGROUND flight simulator.
...
STL = os.path.join(REPO, "fpv85", "cad", "stl")
...
MODEL = dict(name="FR4N8", sub="micro FPV extérieur · 85 mm · 2S", wb=65,
             ...
             frame_z=0.007,
             elec=dict(board=[0,0,6.2], airunit=[0,3,11.2], fpvcam=[0,-9.5,10.4], rxmod=[0,10,19.8], antennas=[0,0,10.4], battery=[0,2,-4.0]),
...
land</textarea>
  ...
  <div id="view">
    <div id="tip"><b>Souris :</b> glisser = orbite · molette = zoom ·
      clic-droit = pan. FPV extérieur : radio <b>ELRS</b>, vidéo
      <b>5,8 GHz analogique</b>, moteurs brushless 2S.</div>
  </div>
</div>
...
const GROUPS = {
  ...
  motors: {label:'Moteurs 0802',            color:'#8b929c'},
  props:  {label:'Hélices 40 mm',           color:'#2f6fed'},
